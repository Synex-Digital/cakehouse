...
const MenuCard = ({ image, title, description, price, link }) => {
  return (
    <li class="card-container xl:w-1/4 lg:w-1/3 sm:w-1/2 w-full pl-[15px] pr-[15px] pb-[30px] All pizza spicy">
      <div class="dz-img-box3 box-hover group style-4 bg-white p-[18px] flex flex-col h-[160px] relative z-[1] overflow-hidden rounded-[10px]">
        <div class="menu-detail flex items-center mb-3">
          ...
          <div class="dz-content">
            <h5 class="title mb-[3px] duration-500">
              <a href="product-detail.html">{title}</a>
            </h5>
            <p>{description}</p>
          </div>
        </div>
        <div class="menu-footer max-w-[110px] mt-auto flex flex-col">
          <span class="text-bodycolor text-[13px]">Regular Price</span>
          <span class="price duration-500 mt-1">${Math.round(price)}</span>
        </div>
        <Link class="detail-btn flex justify-center items-center" to={link}>
